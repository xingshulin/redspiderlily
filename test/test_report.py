...
class ReportTest(TestCase):
    ...
    def test_online_for_first_week(self):
        """----------------Verify behavior-----------------------------"""
        if not ONLINE_TEST_REAL_WORLD:
            return
        global _from, _to
        _from = date(2016, 5, 2)
        _to = date(2016, 5, 9)
    ...
    def test_online_for_second_week(self):
        """----------------Verify behavior-----------------------------"""
        if not ONLINE_TEST_REAL_WORLD:
            return
        global _from, _to
    ...
    def test_generate_report_when_target_duration_has_been_fixed(self, retrieve):
        """----------------Verify behavior-----------------------------"""
        global _from, _to
        if not ONLINE_TEST_GENERATE_REPORT:
            return
        _from = date(2016, 5, 2)
        _to = date(2016, 5, 16)
        result = generate(_from, _to)
    ...
    @mock.patch('module.report.total_peers',
                return_value=3)
    def test_compose_even_week_email_can_zip_senders_and_subjects_into_a_msg_dictionary(self, retrieve):
        """----------------Verify behavior-----------------------------"""
